...
function Payments() {
  ...
  // pre-fill inputs with user data
  ...
  const handleContinue = () => {
    // simple validation
    if (!name || !email) {
      alert("الرجاء ملء الاسم والبريد الإلكتروني قبل المتابعة");
      return;
    }
    // store selection in sessionStorage to pass to fpayments
    sessionStorage.setItem(
      "pendingPayment",
      JSON.stringify({
        feeType: fees.find((f) => f.id === selectedFee)?.title,
        amount: fees.find((f) => f.id === selectedFee)?.amount,
        billingName: name,
        billingEmail: email,
      }),
    );
    navigate("/dashboard/fpayments");
  };
            ...
            {fees.map((fee) => {
              ...
              return (
                ...
                >
                  ...
                  <p
                    className={`font-medium text-sm mt-1 ${isSelected ? "text-green-800" : ""}`}
                  >
                    {fee.amount}
                  </p>
                </button>
